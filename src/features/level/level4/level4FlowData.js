...
export const level4Flow = [
  ...
  },
  {
    ...
    text: [
      '— **Мошенники** могли бы:',
      'получить доступ к **аккаунту** (например, к **госпорталу**),',
...
export const game1Cases = [
  {
    ...
    right: {
      kind: 'sms',
      from: '900',
      text: 'Добрый день, клиент ОрбитаБанка, по вашей карте *4567 заблокирована подозрительная операция, для подтверждения операции позвоните на 8-800-800008',
    },
    real: 'right',
    wrongExplain:
  ...
  {
    title: 'Сообщение от госпортала',
    left: {
      kind: 'notice',
      text: 'ГосПортал: Ваши паспортные данные скоро истекают. Зайдите в личный кабинет на gosportal-info.ru → раздел «Документы» → обновите данные.',
    },
    right: {
      kind: 'notice',
      text: 'ВНИМАНИЕ! Ваш аккаунт на ГосПортале заблокирован. Срочно перейдите по ссылке: gosportal-podtverzhdenie.ru и введите данные. Срок — 24 часа!',
    },
    real: 'left',
    wrongExplain:
      '- Во втором случае имеется эффект **срочности** и **давления**, также имеется **подозрительная ссылка**, которую мошенники сделали похожей на оригинальную gosportal-podtverzhdenie.ru. А также признаками мошенников являются — **угроза блокировки аккаунта** и **неестественные формулировки** в тексте.',
    rightExplain:
      '- Обратите внимание! **Мошенники** часто пытаются украсть ваши **данные** или **доступ** из госпорталов.',
  },
  {
    title: 'Восстановление пароля',
    left: {
      kind: 'email',
      from: 'support@connectu.social',
      subject: 'Смена пароля',
      text: 'Добрый день, от вас поступил запрос на установку нового пароля, для подтверждения операции зайдите на connectu.social/reset',
    },
    right: {
      kind: 'email',
      from: 'security@gmail.com',
      subject: 'Смена пароля',
      text: 'Добрый день, от вас поступил запрос на установку нового пароля, для подтверждения операции зайдите на security.special.com/reset',
    },
    real: 'left',
    wrongExplain:
      '-В правом сообщении подозрительный отправитель с почтовым адресом на **gmail.com** — бесплатном и доступном для всех. Также предлагаемая ссылка ведёт явно **не на connectu.social**.',
    rightExplain:
      'Подозрительный отправитель с адресом на **gmail.com** и ссылка ведёт **не на connectu.social**.',
  },
  {
    ...
    real: 'left',
    wrongExplain:
      '- Тут мошенники поступили хитрее! Они сделали **точную копию** и даже название подобрали. Но **сертификат безопасности** им так просто не получить! Поэтому **замочек** есть только у правильного сайта. Может быть также такой **небезопасный** значок.',
    rightExplain:
  ...
  },
  {
    ...
    left: {
      kind: 'notice',
      text: 'КурьерПлюс: ваша посылка заблокирована на таможне. Для оплаты пошлины перейдите по ссылке courierplus-oplata.ru и введите данные карты.',
    },
    right: {
      kind: 'notice',
      text: 'КурьерПлюс: посылка №12345 прибыла в пункт выдачи. Забрать можно в течение 7 дней.',
    },
    real: 'right',
    wrongExplain:
      '- Левое сообщение содержит подозрительную ссылку (courierplus-oplata.ru вместо официального courierplus.ru), а также требования ввести данные карты на стороннем сайте.',
    rightExplain:
      '- **Мошенники** могут выдавать себя за различные **бренды**, **курьерские доставки**, почту и т. д. Всегда будьте внимательны и **перепроверяйте информацию**!',
  },
]
...
export const game2Cases = [
  {
    ...
    msg:
      '«Банк Орбита: эксклюзивное предложение для вас! Вложи 5 000 руб. — получи 15 000 руб. через неделю! Гарантированная доходность 200 %. Переведи деньги на счёт 4081****1234 и отправь чек в этот чат. Акция действует 2 часа!»',
    options: [
      'А. Переведу деньги на указанный счёт.',
      'Б. Проверю в официальном приложении банка или на сайте orbitabank-demo.ru.',
      'В. Напишу в чат «Спасибо, сейчас сделаю!»',
      'Г. Проигнорирую сообщение.',
    ],
    correct: ['Б. Проверю в официальном приложении банка или на сайте orbitabank-demo.ru.'],
  ...
  },
  {
    title: 'Звонок от банка',
    msg:
      'Звонивший: «Здравствуйте, это служба безопасности СеверБанка. Мы заметили подозрительную активность: на ваше имя взят кредит в размере 300 000 руб. Сейчас вам придёт СМС-код, назовите его, и мы заблокируем заявку».',
    options: [
...
export const fillBlocks = [
  {
    ...
    lines: [
      [
        'Мошенники часто представляются сотрудниками банка и сообщают о якобы',
        'операции или кредите.',
      ],
      ['Они пытаются создать чувство', ', чтобы человек не успел обдумать свои действия.'],
      [
        'Затем они просят назвать',
        'из короткого текстового сообщения на телефоне, чтобы «подтвердить» или «отменить» операцию.',
      ],
      ['Настоящие сотрудники банка не запрашивают такие', '.'],
    ],
  ...
  },
  {
    ...
    lines: [
      [
        'Дропперство — это когда человека просят',
        'деньги, которые получены незаконным путём.',
      ],
      ['Мошенники обещают лёгкий', '.'],
      ['На деле это', 'действия.'],
      ['Человек становится участником', 'схемы.'],
    ],
    bank: ['незаконные', 'хранить', 'заработок', 'безопасной', 'игрой', 'переводить', 'преступной'],
    ans: ['переводить', 'заработок', 'незаконные', 'преступной'],
  },
  {
    title: '3) Заполните пропуски',
    lines: [
      [
        'Дипфейки — это поддельные',
        'или записанные голосовые сообщения, которые создают с помощью искусственного интеллекта.',
      ],
      ['Они могут имитировать', 'настоящего человека.'],
      ['Мошенники используют такие материалы, чтобы у зрителя или слушателя возникло', '.'],
      ['Поэтому важно не доверять только увиденному и услышанному и всегда отдельно', 'информацию.'],
    ],
  ...
  },
  {
    ...
    lines: [
      [
        'Технология бесконтактной оплаты: устройства обмениваются данными на расстоянии в несколько сантиметров без провода. С её помощью можно оплачивать покупки, используя',
        'или банковскую карту без необходимости прикладывать её к платёжному терминалу.',
      ],
      [
        'Однако мошенники могут попытаться получить несанкционированный',
        'к смартфону, планшету или компьютеру пользователя.',
      ],
      [
        'Иногда мошенники убеждают установить на телефон или планшет подозрительные',
        'или перейти по ссылке и выполнить действия, которые подсказывает незнакомец.',
      ],
      [
        'Поэтому важно не передавать своё мобильное устройство или компьютер в руки',
        'людям.',
      ],
    ],
    bank: ['кошелька', 'приложения', 'сообщения', 'вход', 'телефон', 'интернету', 'друзьям', 'доступ', 'незнакомым'],
    ans: ['телефон', 'доступ', 'приложения', 'незнакомым'],
  },
  {
    title: '5) Заполните пропуски',
    lines: [
      [
        'Поддельные сайты в интернете и страницы оплаты часто предлагают товары по слишком',
        'ценам, чтобы привлечь внимание покупателя.',
      ],
      [
        'Они могут выглядеть как известные',
        'или официальные интернет-витрины брендов одежды, обуви и электроники.',
      ],
      [
        'Пользователя торопят с оплатой, создавая ощущение очень',
        'времени на размышление.',
      ],
      [
        'После ввода реквизитов банковской карты или после отправки оплаты деньги могут быть',
        '.',
      ],
    ],
    bank: ['нереальные', 'украдены', 'люди', 'ограниченного', 'низким', 'высокие', 'проверенные', 'магазины', 'безопасного'],
    ans: ['низким', 'магазины', 'ограниченного', 'украдены'],
  },
]
